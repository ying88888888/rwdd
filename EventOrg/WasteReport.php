...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    } elseif (!is_numeric($quantity) || !is_numeric($collected)) {
        $error = "Quantity and Collected must be numbers.";
    } else {
        $stmt = $conn->prepare("INSERT INTO waste_reports (event_id, waste_type, quantity, collected, collection_method, disposal_method) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("isddss", $event_id, $waste_type, $quantity, $collected, $collection_method, $disposal_method);
      ...
      <form method="POST" action="WasteReport.php?id=<?php echo $event_id; ?>" class="waste-form">
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" step="0.01" name="quantity" id="quantity" placeholder="Enter quantity" required>
          </div>

          <div class="form-group">
            <label for="collected">Collected </label>
            <input type="number" step="0.01" name="collected" id="collected" placeholder="Enter collected amount" required>
          </div>
        </div>
