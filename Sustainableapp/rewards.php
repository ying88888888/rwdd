...
$pointsHistoryStmt = $conn->prepare("
    SELECT pointsHistory_activity, pointsHistory_points, pointsHistory_time
    FROM pointshistory
    WHERE user_id = ?
      AND pointsHistory_points > 0
    ORDER BY pointsHistory_time DESC
");
$pointsHistoryStmt->bind_param("i", $participant_id);
  ...
  <section class="rewards-section">
    ...
    <div class="table-wrap">
      <table class="rewards-table">
        ...
        <tbody>
          <?php if ($pointsHistoryResult->num_rows > 0): ?>
            <?php while ($row = $pointsHistoryResult->fetch_assoc()): ?>
              <tr>
                <td><?= htmlspecialchars($row['pointsHistory_activity']) ?></td>
                <td><?= (int)$row['pointsHistory_points'] ?> points</td>
                <td><?= date("d F Y", strtotime($row['pointsHistory_time'])) ?></td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
            <tr>
              <td colspan="3">No points earned yet.</td>
            </tr>
          <?php endif; ?>
        </tbody>
      </table>
