...
$sqlEventPoints = "SELECT SUM(pointsHistory_points) as total FROM pointshistory WHERE pointsHistory_activity NOT LIKE '%Redeemed reward%' AND pointsHistory_activity NOT LIKE '%Feedback%'";
$resEvent = mysqli_query($dbConn, $sqlEventPoints);
$totalEvent = (int)(mysqli_fetch_assoc($resEvent)['total']?? 0);

$sqlFeedbackPoints = "SELECT SUM(pointsHistory_points) as total FROM pointshistory WHERE pointsHistory_activity LIKE '%Feedback%'";
